...
Check memory/patterns.json for existing conversion patterns:
```bash
# Load successful conversion patterns with high confidence
jq '.[] | select(.category == "conversion" and .confidence > 0.7)' memory/patterns.json
```
...
Leverage MCP tools for analysis:
- **Serena MCP**: `mcp__serena__search_for_pattern` - Find code patterns in the command
- **Calculator MCP**: `mcp__calculator__calculate` - Calculate complexity metrics
...
Update memory/patterns.json with successful conversion:
```json
{
  "conversion_[command_name]": {
    "name": "TodoWrite Conversion Pattern",
    "confidence": 0.9,
    "usage_count": 1,
    "last_used": "[timestamp]",
    "mcp_tools": ["serena", "calculator"]
  }
}
...
Load context efficiently using 4-Index System:
- **PROJECT_INDEX.json** (~160KB) for code structure
- **memory/patterns.json** for proven patterns
- **memory/agent-groups.json** for agent capabilities
...
**Task Delegation**: Use Task tool with focused analysis and MCP integration:
