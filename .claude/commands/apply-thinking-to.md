...
Before enhancement, check existing patterns:
```bash
# Load proven thinking patterns from memory
jq '.[] | select(.category == "thinking_pattern" and .confidence > 0.8)' memory/patterns.json
```

Leverage MCP tools for analysis:
- **Serena MCP**: `mcp__serena__search_for_pattern` - Find existing thinking patterns in codebase
- **Calculator MCP**: `mcp__calculator__calculate` - Calculate complexity scores and confidence metrics
- **Context7 MCP**: `mcp__context7__get-library-docs` - Research best practices for prompt engineering

Update memory after successful enhancement:
```json
// memory/patterns.json
{
  "enhanced_thinking_[prompt_name]": {
    "name": "Enhanced Thinking Pattern",
    "category": "thinking_pattern",
    "confidence": 0.9,
    "usage_count": 1,
    "last_used": "[timestamp]",
    "mcp_tools": ["serena", "calculator", "context7"]
  }
}
